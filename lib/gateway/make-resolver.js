...
function getDirectiveTypeSelection(node, directiveName) {
...
function getDirectiveRequiresField(selections, service, type, schema, typeToServiceMap, serviceMap, typeFieldsToService) {
  console.log('--- getDirectiveFieldSelection')

  console.log('selections', selections.map(s => s.name.value))
  console.log('service', '***', service.name, '***')

  console.log('type', type.name)
  console.log('type:fields', type.astNode.fields
    .map(f => ({
      name: f.name.value,
      directives: JSON.stringify(f.directives)
    })))
  ...
  if (service.name == 'host') {
    // concat fields
    requires.push(parse(`{ size }`).definitions[0].selectionSet.selections[0])
  }
  /*
  for (const selection of selections) {
    if(hasRequires(selection.name.value, type.extensionASTNodes[0].fields[0].name.value)) {
      // type.extensionASTNodes[0].fields[0].directives[0].name.value === 'requires'

      // requires.push type.extensionASTNodes[0].fields[0].directives[0].arguments[0].value.value
      parse(`{ ${directive.arguments[0].value.value} }`)
    }
  }
  */

  // type.extensionASTNodes[0].fields[0].name.value

  console.log('######') // , typeToServiceMap, serviceMap, typeFieldsToService)
  // look for selections directives
  // add size to host service!

  return requires
}

function setupServiceTypeFields(selections, service, type, schema, typeToServiceMap, serviceMap, typeFieldsToService) {
  console.log('--- setupServiceTypeFields')
  return [
    ...selections.filter(selection => selection.kind === Kind.INLINE_FRAGMENT || selection.kind === Kind.FRAGMENT_SPREAD || service.typeMap[type].has(selection.name.value))
      .map(selection => {
        if (selection.selectionSet && selection.selectionSet.selections) {
          if (selection.kind === Kind.INLINE_FRAGMENT) {
            ...
            for (const field of Object.values(inlineFragmentType.getFields())) {
              requiredFields.push(...getDirectiveTypeSelection(field, 'requires'))
            }
            ...
            return {
              ...
              selectionSet: {
                ...
                selections: setupServiceTypeFields([...selection.selectionSet.selections, ...requiredFields], service, inlineFragmentType, schema, typeToServiceMap, serviceMap, typeFieldsToService)
              }
            }
          }
          ...
          return {
            ...
            selectionSet: {
              kind: Kind.SELECTION_SET,
              selections: setupServiceTypeFields([...selection.selectionSet.selections, ...requiredFields], service, fieldType, schema, typeToServiceMap, serviceMap, typeFieldsToService)
            }
          }
        }
    ...
    ...getDirectiveTypeSelection(type, 'key'),
    ...getDirectiveRequiresField(selections, service, type, schema, typeToServiceMap, serviceMap, typeFieldsToService),
  ]
}
...
function collectFragmentsToInclude(usedFragments, fragments, service, schema) {
  ...
  for (const fragmentName of usedFragments) {
    ...
    const selections = setupServiceTypeFields(fragment.selectionSet.selections, service, fragment.typeCondition.name.value, schema) // @todo? typeToServiceMap, serviceMap, typeFieldsToService
 ...
 *
 */
function makeResolver({ service, createOperation, transformData, isQuery, isReference, isSubscription, typeToServiceMap, serviceMap, typeFieldsToService }) {
  ...
  return async function (parent, args, context, info) {
    ...
    if (cached) {
      ...
      // @todo ? typeToServiceMap, serviceMap, typeFieldsToService
      const selections = fieldNodes[0].selectionSet ? setupServiceTypeFields(fieldNodes[0].selectionSet.selections, service, type, schema, typeToServiceMap, serviceMap, typeFieldsToService) : []
    ...
    const response = await reply[kEntityResolvers][`${service.name}Entity`]({
      query,
      variables,
      originalRequestHeaders: reply.request.headers,
      context,
