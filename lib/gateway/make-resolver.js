...
function setupServiceTypeFields (selections, service, type, schema) {
  console.log('!!', service.extensionTypeMap)
  return [
    ...selections.filter(selection => {
      console.log('selection', selection.name.value, selection.kind)
      return selection.kind === Kind.INLINE_FRAGMENT || selection.kind === Kind.FRAGMENT_SPREAD || service.typeMap[type].has(selection.name.value)
    })
      .map(selection => {
      if (selection.selectionSet && selection.selectionSet.selections) {
        if (selection.kind === Kind.INLINE_FRAGMENT) {
          ...
          for (const field of Object.values(inlineFragmentType.getFields())) {
            ...
            console.log('requiredFields #1', requiredFields)
          }
          ...
          return {
            ...
            selectionSet: {
              ...
              selections: setupServiceTypeFields([...selection.selectionSet.selections, ...requiredFields], service, inlineFragmentType, schema)
            }
          }
        }
        ...
        return {
          ...
          selectionSet: {
            kind: Kind.SELECTION_SET,
            selections: setupServiceTypeFields([...selection.selectionSet.selections, ...requiredFields], service, fieldType, schema)
          }
        }
      }
...
function collectFragmentsToInclude (usedFragments, fragments, service, schema) {
  ...
  for (const fragmentName of usedFragments) {
    ...
    const selections = setupServiceTypeFields(fragment.selectionSet.selections, service, fragment.typeCondition.name.value, schema)
...
function makeResolver ({ service, createOperation, transformData, isQuery, isReference, isSubscription }) {
  return async function (parent, args, context, info) {
    ...
    if (cached) {
      ...
      const selections = fieldNodes[0].selectionSet ? setupServiceTypeFields(fieldNodes[0].selectionSet.selections, service, type, schema) : []
